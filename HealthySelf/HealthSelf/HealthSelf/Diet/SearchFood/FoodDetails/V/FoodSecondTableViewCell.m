...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        self.glycemicInfoDataLabel = [[UILabel alloc] init];
        self.glycemicInfoDataLabel.text = @"甘油指数:";
        self.glycemicInfoDataLabel.textColor = [UIColor grayColor];
        self.glycemicInfoDataLabel.font = [UIFont systemFontOfSize:12];
        
        self.giLabel = [[UILabel alloc] init];
        self.giLabel.text = @"升糖指数: 55.0 中GI";
        self.giLabel.textColor = [UIColor grayColor];
        self.giLabel.font = [UIFont systemFontOfSize:12];
        
        self.glLabel = [[UILabel alloc] init];
        self.glLabel.text = @"升糖复合: 55.0 中GL";
        self.glLabel.textColor = [UIColor grayColor];
        self.glLabel.font = [UIFont systemFontOfSize:12];
        ...
        self.foodColorView2 = [[UIView alloc] init];
        self.foodColorView2.layer.masksToBounds = YES;
        self.foodColorView2.layer.cornerRadius = 2.5;
        self.foodColorView2.backgroundColor = [UIColor redColor];
        
        self.recordButton = [UIButton buttonWithType:UIButtonTypeRoundedRect];
        self.recordButton.layer.masksToBounds = YES;
        self.recordButton.layer.cornerRadius = 18.0;
        [self.recordButton setTitle:@"记录饮食" forState:UIControlStateNormal];
        ...
        [self.backWhiteView addSubview:self.recordButton];
        // GRayView
        ...
        [self.backGrayView addSubview:self.glycemicInfoDataLabel];
        [self.backGrayView addSubview:self.giLabel];
        [self.backGrayView addSubview:self.glLabel];
        [self.backGrayView addSubview:self.foodColorView];
        [self.backGrayView addSubview:self.foodColorView2];
    }
    return self;
}
- (void)layoutSubviews {
    ...
    [self.glycemicInfoDataLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(@20);
        make.left.equalTo([self.lineOnGrayBackView mas_right]).offset(10);
    }];
    [self.giLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo([self.glycemicInfoDataLabel mas_bottom]).offset(30);
        make.left.equalTo([self.glycemicInfoDataLabel mas_left]).offset(10);
    }];
    [self.glLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo([self.giLabel mas_bottom]).offset(10);
        make.left.equalTo([self.glycemicInfoDataLabel mas_left]).offset(10);
    }];
    [self.foodColorView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo([self.giLabel mas_top]).offset(5);
        make.width.equalTo(@5);
        make.height.equalTo(@5);
        make.left.equalTo([self.glycemicInfoDataLabel mas_left]).offset(2);
    }];
    [self.foodColorView2 mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo([self.glLabel mas_top]).offset(5);
        make.width.equalTo(@5);
        make.height.equalTo(@5);
        make.left.equalTo([self.glycemicInfoDataLabel mas_left]).offset(2);
    }];
    [self.recordButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo([self.fiberDietaryLabel mas_bottom]).offset(30);
        make.left.equalTo([self.contentLabel mas_left]).offset(-100);
